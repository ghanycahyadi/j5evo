...
import { SPKLU_DATABASE } from "./SpkluData";
import type { SpkluDbItem } from "./SpkluData";
            ...
            <div className="bg-[#0c0d12] border border-slate-900 rounded-xl p-3.5 space-y-3 select-none">
              ...
              <div className="w-full">
                <div className="flex flex-wrap items-start justify-center gap-y-6 gap-x-2 py-2 w-full">
                  ...
                  {activeTimeline.filter(item => item.type === "spklu" && item.isCharging).map((item) => (
                    <React.Fragment key={item.id}>
                      ...
                      <div className="flex items-center justify-center h-10 text-zinc-600 px-0.5" style={{ minWidth: '12px' }}>
                        <svg className={`w-3.5 h-3.5 text-emerald-500/50 flex-shrink-0 ${isExporting ? "" : "animate-pulse"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                  ...
                  <div className="flex items-center justify-center h-10 text-zinc-600 px-0.5" style={{ minWidth: '12px' }}>
                    <svg className={`w-3.5 h-3.5 text-cyan-500/50 flex-shrink-0 ${isExporting ? "" : "animate-pulse"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                    </svg>
                  </div>
