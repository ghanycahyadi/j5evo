...
export default function App() {
  ...
  const [eventWinners, setEventWinners] = useState<Record<string, WinnerLog[]>>(() => {
    try {
      const stored = localStorage.getItem("j5_event_winners");
  ...
  useEffect(() => {
    localStorage.setItem("j5_event_winners", JSON.stringify(eventWinners));
  }, [eventWinners]);
  ...
  {/* Optional Field: Tanggal Lahir */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/70 backdrop-blur-xs animate-fadeIn overflow-y-auto">
          <div className="bg-white rounded-3xl border border-zinc-200 shadow-2xl max-w-2xl w-full overflow-hidden my-8 flex flex-col relative max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 flex flex-col">
              ...
              {/* Event Winners (Pemenang Undian) */}
              {eventWinners[selectedEvent.id] && eventWinners[selectedEvent.id].length > 0 && (
                <div className="border-t border-zinc-200 pt-6 space-y-3">
                  <div className="flex items-center gap-1.5 text-amber-600">
                    <Gift className="w-4 h-4 text-rose-500 animate-pulse" />
                    <h4 className="font-extrabold text-xs uppercase tracking-wider font-mono">🏆 Daftar Pemenang Undian DoorPrize ({eventWinners[selectedEvent.id].length})</h4>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {eventWinners[selectedEvent.id].map((winner) => (
                      <div key={winner.id} className="flex items-center justify-between p-3.5 bg-amber-500/5 border border-amber-300 rounded-2xl">
                        <div className="text-left space-y-0.5">
                          <p className="text-xs font-black text-zinc-950 leading-tight">{winner.name}</p>
                          <p className="text-[10px] text-[#005c56] font-bold flex items-center gap-1">
                            <Award className="w-3.5 h-3.5 text-rose-500" /> {winner.prize}
                          </p>
                        </div>
                        <span className="text-[9px] font-mono font-medium text-zinc-400 shrink-0">{winner.time}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="border-t border-zinc-200 pt-6 space-y-4">
                ...
                <div className="bg-zinc-50 border border-zinc-200/80 rounded-2xl p-4.5 space-y-2.5 text-left">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    {adminAuthenticated && registrations.filter(r => r.eventId === selectedEvent.id).length > 0 && (
                      <button
                        type="button"
                        onClick={() => setIsWheelOpen(true)}
                        className="px-2.5 py-1 bg-rose-500 hover:bg-rose-600 text-white text-[10px] font-extrabold uppercase rounded-lg shadow-sm hover:scale-[1.02] flex items-center gap-1 transition cursor-pointer select-none active:scale-[0.98]"
                      >
      ...
      {/* 🔮 EVENT PARTICIPANTS LUCKY DRAW WHEEL OVERLAY MODAL */}
          ...
          <div className="bg-white rounded-3xl border border-zinc-200 shadow-2xl max-w-5xl w-full p-4 md:p-6 my-8 relative animate-scaleUp overflow-hidden max-h-[95vh] flex flex-col">
            <div className="overflow-y-auto flex-1 pr-1">
              {(() => {
                ...
                const participants = eventRegs.map((reg, idx) => {
                  const matched = members.find(m => m.id === reg.memberId);
                  return {
                    id: reg.id || `temp-${idx}`,
                    name: matched ? matched.name : `Tamu #${idx + 1}`,
                ...
                return (
                  <LuckyDrawWheel
                    ...
                    showFeedback={(text, isError) => showFeedback(text, isError)}
                    eventTitle={selectedEvent.title}
                    onClose={() => setIsWheelOpen(false)}
                    winnerLogs={eventWinners[selectedEvent.id] || []}
                    onWinnerClaimed={(winner) => {
                      setEventWinners(prev => ({
                        ...prev,
                        [selectedEvent.id]: [winner, ...(prev[selectedEvent.id] || [])]
                      }));
                    }}
                    onClearWinners={() => {
                      setEventWinners(prev => ({
                    ...
                    onRemoveSingleWinner={(winnerId) => {
                      setEventWinners(prev => ({
                        ...prev,
                        [selectedEvent.id]: (prev[selectedEvent.id] || []).filter(w => w.id !== winnerId)
                      }));
                    }}
                  />
                );
      ...
      {isPromoOpen && homeContent?.promoActive && homeContent?.promoImage && (
        <div 
          ...
          className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/45 backdrop-blur-[2px] animate-fadeIn cursor-pointer"
        >
